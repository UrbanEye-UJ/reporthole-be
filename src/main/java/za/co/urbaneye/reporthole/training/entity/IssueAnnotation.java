...
public class IssueAnnotation {
    ...
    /** Null for an auto-generated annotation (see {@link #autoGenerated}) — there is no human annotator. */
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "ANNOTATION_ANNOTATED_BY", nullable = true)
    private User annotatedBy;

    /**
     * True when this box was drawn automatically from the custom model's own bounding-box output
     * at incident-creation time, rather than by a security admin in the manual annotation tool.
     */
    @Column(name = "ANNOTATION_AUTO_GENERATED", nullable = false)
    @Builder.Default
